Component({
  ...
  landscape(res){
    let that = this;
    console.log("对相片做横屏处理");
    wx.getImageInfo({
      src: res.tempFilePaths[0],
      success (res) {
        let DRAW_IMAGE_W= res.width
        let IMG_REAL_W = res.width
        let IMG_REAL_H = res.height
        let IMG_RATIO = Number((IMG_REAL_W / IMG_REAL_H).toFixed(5))
           // 如果图片宽度大于等于高度（横向）
        if (IMG_RATIO >= 1) {
              //在用户旋转的基础上旋转90°
              that.setAngle(that.data.angle+=90);
          console.log("如果图片宽度大于等于高度（横向）");
           }else{
              console.log("如果图片宽度大于等于高度（纵向）");
           }
      }
    })
  },
